...
async function updateDefaultParameterRange(req, res) {
    try {
        const { id } = req.params;
        const sanitizedData = sanitizeDefaultParameterRange(req.body);
        const response = await updateDefaultParameterRangeDb(id, sanitizedData);
        return response.statusCode === 200
            ? successResponse(res, response)
            : errorResponse(res, response);
    } catch (error) {
...
async function getAllParameterRangesByParameterId(req, res) {
    try {
        const { parameterId, parameterRangeId } = req.params;
...
async function getSingleParameterRange(req, res) {
    try {
        ...
        const response = await getSingleParameterRangeByIdDb(parameterId);
        return response.length > 0
            ? successResponse(res, response)
            : errorResponse(res, response);
    } catch (error) {
...
async function deleteParameterRange(req, res) {
    try {
        const { id } = req.params;
        const response = await deleteDefaultParameterRangeDb(id);
        return response.statusCode === 200
            ? successResponse(res, response)
            : errorResponse(res, response);
    } catch (error) {
